...
4. **Create an isolated worktree:**
   ```bash
   WORKTREE_PATH=".agent/worktrees/agent-${SPECIES}-${VERSION}"
   git worktree add "$WORKTREE_PATH" "$BASELINE"
   ```
   Prefer `.agent/worktrees/agent-*` (already managed by `gad worktree list/clean`)
...
5. **Write the session marker (status: running):**
   ```json
   {
     ...
     "runtime": "claude-code",
     "baseline": "HEAD",
     "worktreePath": ".agent/worktrees/agent-escape-the-dungeon-v7",
...
`.agent/worktrees/agent-escape-the-dungeon-v7`, writes marker at
...
`gad generation preserve escape-the-dungeon v7 --from .agent/worktrees/agent-escape-the-dungeon-v7`,
